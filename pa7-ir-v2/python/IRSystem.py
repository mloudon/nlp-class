    ...
    def index(self):
        ...
        for d in range (len(self.docs)):
            for word in self.docs[d]:
                doc_list.append((word,d)) 
                
        doc_list = list(set(doc_list))
        sorted(doc_list, key=itemgetter(0,1))      
        
        inv_index= {}
        
        for entry in doc_list:
            if entry[0] not in inv_index:
                inv_index[entry[0]] = [entry[1]]
            else:
                inv_index[entry[0]].append(entry[1]) 
                
        for key in inv_index:
            print '%s:%s'%(key, inv_index[key])
